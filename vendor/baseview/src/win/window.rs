...
use winapi::um::winuser::{
    ...
    GetAsyncKeyState, GetCapture, GetDpiForWindow, GetMessageW, GetSystemMetrics, GetWindowLongPtrW, LoadCursorW, PostMessageW, RegisterClassW,
...
use super::mouse::{Buttons, BUTTONS};
...
unsafe fn wnd_proc_inner(
    ...
    // A cancellation must reach the UI before a later press starts a new drag.
    window_state.flush_button_releases();
    match msg {
        WM_CAPTURECHANGED => {
            window_state.cancel_buttons();
            Some(0)
        }
                ...
                let event = match msg {
                    ...
                    WM_LBUTTONUP | WM_MBUTTONUP | WM_RBUTTONUP | WM_XBUTTONUP => {
                        ...
                        if !pressed.remove(button) {
                            // Already delivered by capture cancellation.
                            return None;
                        }
                        // ReleaseCapture sends WM_CAPTURECHANGED synchronously.
                        // Clear this button first, so that notification cannot
                        // synthesize a second release for a normal mouse-up.
                        window_state.pressed_buttons.set(pressed);
                        if pressed.is_empty() && GetCapture() == hwnd {
                            ReleaseCapture();
                        }
        ...
        WM_TIMER => {
            ...
            if wparam == WIN_FRAME_TIMER {
                // A background/embedded window can miss mouse-up outside its
                // bounds. Consult native state, not Vizia's stale event cache.
...
impl WindowState {
    fn queue_button_releases(&self, released: Buttons) {
        ...
        let mut pending = self.pending_releases.get();
        let wake = pending.is_empty();
        pending.merge(released);
        self.pending_releases.set(pending);
        if wake {
            unsafe { PostMessageW(self.hwnd, BV_RELEASE_LOST_BUTTONS, 0, 0); }
        }
    }

    fn cancel_buttons(&self) {
        let released = self.pressed_buttons.replace(Buttons::default());
        self.queue_button_releases(released);
    }

    fn flush_button_releases(&self) {
    ...
    unsafe fn reconcile_buttons(&self) {
        let mut pressed = self.pressed_buttons.get();
        if pressed.is_empty() {
            return;
        }
        ...
        let mut released = Buttons::default();
        // GetAsyncKeyState uses physical buttons; mouse messages use the
        // user's logical left/right assignment.
        let swapped = GetSystemMetrics(SM_SWAPBUTTON) != 0;
        for (button, key) in BUTTONS.iter().copied().zip([
            if swapped { VK_RBUTTON } else { VK_LBUTTON },
            VK_MBUTTON,
            if swapped { VK_LBUTTON } else { VK_RBUTTON },
            VK_XBUTTON1, VK_XBUTTON2,
        ]) {
            if pressed.contains(button) && GetAsyncKeyState(key) >= 0 {
                pressed.remove(button);
                released.insert(button);
            }
        }
        self.pressed_buttons.set(pressed);
        self.queue_button_releases(released);
        if pressed.is_empty() && GetCapture() == self.hwnd {
            ReleaseCapture();
        }
    }
